...
var audioProfile = 'high_quality_stereo';
...
function initClientAndJoinChannel(agoraAppId, channelName, token) {
  // init Agora SDK
...
function joinChannel(channelName,token) {
  var token = token ==null? generateToken(): token;
...
function getCameraDevices() {
  ...
  client.getCameras(function (cameras) {
    ...
    cameras.forEach(function (camera, i) {
      ...
      $("#camera-list").append(
        '<a class="dropdown-item" id="' + optionId + '">' + optionId + "</a>"
      );
    });
...
function changeStreamSource(deviceIndex, deviceType) {
  ...
  if (deviceType === "video") {
    localStreams.camera.stream.getVideoTrack().stop();
